...
CREATE TABLE exam_item_revisions (
...
CREATE INDEX ix_exam_item_revisions_item_id ON exam_item_revisions(item_id);
CREATE INDEX ix_exam_item_revisions_created_at ON exam_item_revisions(created_at);


CREATE TABLE exam_item_reports (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    item_id     UUID NOT NULL REFERENCES exam_items(id) ON DELETE CASCADE,
...
CREATE INDEX ix_exam_item_reports_status ON exam_item_reports(status);
CREATE INDEX ix_exam_item_reports_item_id ON exam_item_reports(item_id);
