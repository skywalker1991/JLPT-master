...
ATOM_FORMAT_RULES = """\
词汇 base/meaning 分层要求（最重要，请严格遵守）：
- base 与 meaning 必须成对：meaning 写的是 base 辞書形**本身**的含义，
  绝不能写成句中活用形的含义。活用形的含义写进 surface_meaning。
- 正确示例：句中出现「尊ばれてきた」
    surface=尊ばれた  base=尊ぶ  reading=とうとぶ
    meaning=尊敬、珍视          ← 辞書形本身的意思
    surface_meaning=受到尊重     ← 被动形在本句中的意思
- 错误示例（禁止）：base=尊ぶ 却 meaning=受到尊重（这是 尊ばれる 的意思）；
  base=手軽 却 meaning=简单地（这是 手軽に 的意思）
- base 必须规范化到辞書形：「手軽に」→「手軽」、「煮付け」→「煮付ける」、
  「もの珍しさ」→「もの珍しい」、「やらない」→「やる」。
  不得把活用形或派生形当作独立词条。
...
例句格式要求（词汇和语法一致）：
- 必填，且必须带中文翻译，格式：「例句」→中文翻译
- 例：「申込書にコピーを添付した。」→在申请表上附上了复印件。"""
